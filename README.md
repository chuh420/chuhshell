...
enter launches the selected application, ctrl+b focuses the list, ctrl+f
focuses the search field, and escape closes it.
...
`Mod+D` to `chuhshell launcher` and `Mod+Shift+D` to `chuhshell manage`.
...
`modules.rs` runs the system pollers. `apps.rs` reads desktop entries and the
